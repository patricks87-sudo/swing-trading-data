...
Ein 25-Minuten-Toleranzfenster faengt verspaetete oder ausgefallene
Cron-Ticks ab: der Workflow laeuft alle 5 Minuten, faellt also ein Tick
aus oder kommt er verspaetet, springt automatisch der naechste Tick ein,
statt dass ein ganzer Lauf komplett ausfaellt (Ursache 1).

Ein Dedupe-Check verhindert dabei, dass derselbe Lauf durch die
haeufigeren Ticks mehrfach innerhalb desselben Zeitfensters ausgefuehrt
wird (er prueft dazu generated_at_utc in der jeweiligen *_latest.json).
"""
...
def pick_run(now_berlin):
    for run, (hour, minute) in TARGETS.items():
        ...
        in_window = 0 <= age_minutes <= GRACE_MINUTES
